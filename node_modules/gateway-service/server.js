    ...
    <body class="bg-slate-50 min-h-screen flex flex-col font-sans">
        ...
        <div class="bg-white border-b border-slate-200 px-6 py-4 flex justify-between items-center max-w-7xl mx-auto w-full">
            <div class="font-bold text-lg text-slate-800 tracking-tight">
                Manthan <span class="text-slate-300">/</span> <span class="text-cyan-700 font-semibold">Utthan Gateway</span>
            </div>
            <div class="flex items-center gap-4">
                <button onclick="switchTab('user')" id="tabUser" class="text-xs font-bold uppercase tracking-wider text-cyan-600 border-b-2 border-cyan-600 pb-0.5">User Registry</button>
                <button onclick="switchTab('status')" id="tabStatus" class="text-xs font-bold uppercase tracking-wider text-slate-400 hover:text-slate-600">Track Status</button>
                <button onclick="switchTab('admin')" id="tabAdmin" class="text-xs font-bold uppercase tracking-wider text-slate-400 hover:text-slate-600">Admin Panel</button>
            </div>
        ...
        <main class="flex-1 flex items-center justify-center p-6">
            <div id="authContainer" class="bg-white border border-slate-200/80 rounded-2xl p-8 max-w-md w-full shadow-xl shadow-slate-100 transition-all duration-300">
                ...
                <div id="dynamicFormTarget" class="space-y-5">
                    <h3 class="text-xl font-bold text-slate-800 mb-2 text-center">Sign in to your account</h3>
        ...
        <script>
            ...
            function switchTab(mode) {
                ...
                document.getElementById('tabUser').className = mode === 'user' ? 'text-xs font-bold uppercase tracking-wider text-cyan-600 border-b-2 border-cyan-600 pb-0.5' : 'text-xs font-bold uppercase tracking-wider text-slate-400 hover:text-slate-600';
                document.getElementById('tabStatus').className = mode === 'status' ? 'text-xs font-bold uppercase tracking-wider text-cyan-600 border-b-2 border-cyan-600 pb-0.5' : 'text-xs font-bold uppercase tracking-wider text-slate-400 hover:text-slate-600';
                document.getElementById('tabAdmin').className = mode === 'admin' ? 'text-xs font-bold uppercase tracking-wider text-cyan-600 border-b-2 border-cyan-600 pb-0.5' : 'text-xs font-bold uppercase tracking-wider text-slate-400 hover:text-slate-600';
                    ...
                    try {
                        ...
                        if(data.success) {
                            ...
                            document.getElementById('otp1Form').addEventListener('submit', async (eO) => {
                                ...
                                // 🟢 FIX: Hits /api/auth/verify-reg to match your active backend definition perfectly
